...
class CrystalTransformer(nn.Module):
    ...
    def forward(self, G, XYZ, A, W, M, is_train=True):
        ...
        # 4. Coordinate Embeddings (hX, hY, hZ)
        X, Y, Z = XYZ[:,:,0], XYZ[:,:,1], XYZ[:,:,2] 
        
        def fourier_encode(coords, Nf):
            # [CRITICAL FIX] 
            # JAX implementation interleaves sin/cos: [sin(f1), cos(f1), sin(f2), cos(f2)...]
            # Previous PyTorch implementation concatenated: [sin_all, cos_all] -> WRONG ORDER for loaded weights
            ...
            # Stack to (B, N, Nf, 2) then flatten last two dims to (B, N, Nf*2)
            # This creates [s1, c1, s2, c2, ...] sequence
            return torch.stack([sines, cosines], dim=-1).flatten(start_dim=-2)
        ...
        # 6. Attention Mask & Transformer Layers
        seq_len = 5 * n_atoms
        ...
        # --- Masking Logic (Refined) ---
        
        # (1) Constraint: W_0 <= W_1 <= W_2
        ...
        # JAX uses -1e10 for invalid positions.
        ...
        # (2) Enhance pad atom probability if previous was type 0
        idx_0_mask = torch.zeros_like(w_logit, dtype=torch.bool)
        idx_0_mask[..., 0] = True
        w_is_0_mask = (W == 0).unsqueeze(-1).expand_as(w_logit)
        
        # Set to large value (1e10) directly rather than adding, matching JAX "where" logic safer
        w_logit = torch.where(idx_0_mask & w_is_0_mask, torch.tensor(1e10, device=device), w_logit)
        w_logit = torch.log_softmax(w_logit, dim=-1)
        
        # (3) Mask out positions > w_max
